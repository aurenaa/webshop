...
@WebServlet("/products/upload-image")
@MultipartConfig
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if (productId == null || productId.isEmpty()) {
                ...
                out.write("{\"error\":\"Missing product ID\"}");
                return;
            }
            ...
            String savedFileName = productDAO.saveProductImage(productId, fileContent, fileName,
                    ctx.getRealPath(""));

            response.setStatus(HttpServletResponse.SC_OK);
            response.setContentType("application/json");
            out = response.getWriter();
            out.write("{\"productImage\":\"" + savedFileName + "\"}");
